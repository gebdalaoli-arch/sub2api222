...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	clientTokenWalletRepository := repository.NewClientTokenWalletRepository(client, db)
	...
	clientTokenChannelResolver := service.ProvideClientTokenChannelResolver(channelService)
	clientTokenBillingService := service.NewClientTokenBillingService(clientTokenWalletRepository, clientTokenChannelResolver)
	billingCacheService := service.ProvideBillingCacheService(billingCache, userRepository, userSubscriptionRepository, apiKeyRepository, configConfig, clientTokenBillingService)
	...
	redeemService := service.NewRedeemService(redeemCodeRepository, userRepository, subscriptionService, redeemCache, billingCacheService, client, clientTokenBillingService, apiKeyAuthCacheInvalidator)
	...
	clientBillingHandler := handler.NewClientBillingHandler(clientTokenBillingService)
	...
	adminUserHandler := handler.ProvideAdminUserHandler(adminService, concurrencyService, clientTokenBillingService)
	...
	desktopUpdateService := service.ProvideDesktopUpdateService(settingRepository)
	desktopUpdateHandler := handler.ProvideAdminDesktopUpdateHandler(desktopUpdateService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, desktopUpdateHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, paymentHandler)
	...
	desktopSessionRepository := repository.NewDesktopSessionRepository(client)
	groupService := service.NewGroupService(groupRepository, apiKeyAuthCacheInvalidator)
	v := service.ProvideDesktopSessionNow()
	v2 := service.ProvideDesktopSessionSigningKey(configConfig)
	desktopSessionService := service.NewDesktopSessionService(desktopSessionRepository, userService, groupService, subscriptionService, v, v2)
	desktopHandler := handler.ProvideDesktopHandler(desktopSessionService)
	handlerDesktopUpdateHandler := handler.ProvideDesktopUpdateHandler(desktopUpdateService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, clientBillingHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, desktopHandler, handlerDesktopUpdateHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, idempotencyCoordinator, idempotencyCleanupService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService)
	apiKeyAuthMiddleware := middleware.NewAPIKeyAuthMiddleware(apiKeyService, subscriptionService, clientTokenBillingService, configConfig)
	desktopRuntimeAuthAPIKeyStore := middleware.ProvideDesktopRuntimeAuthAPIKeyStore(apiKeyRepository)
	desktopRuntimeAuthMiddleware := middleware.ProvideDesktopRuntimeAuthMiddleware(desktopSessionService, userService, groupService, subscriptionService, desktopRuntimeAuthAPIKeyStore)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, desktopRuntimeAuthMiddleware, apiKeyService, clientTokenBillingService, subscriptionService, opsService, settingService, redisClient)
