...
// The DesktopSessionFunc type is an adapter to allow the use of ordinary function as a Querier.
type DesktopSessionFunc func(context.Context, *ent.DesktopSessionQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f DesktopSessionFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.DesktopSessionQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.DesktopSessionQuery", q)
}

// The TraverseDesktopSession type is an adapter to allow the use of ordinary function as Traverser.
type TraverseDesktopSession func(context.Context, *ent.DesktopSessionQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseDesktopSession) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseDesktopSession) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.DesktopSessionQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.DesktopSessionQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.DesktopSessionQuery:
		return &query[*ent.DesktopSessionQuery, predicate.DesktopSession, desktopsession.OrderOption]{typ: ent.TypeDesktopSession, tq: q}, nil
