...
func resolveConvergedInstallationID(account *Account, seeds ...string) string {
	...
	if seed == "" {
		// Preserve the legacy account-ID derivation for accounts without a managed seed.
...
func resolveConvergedSessionID(identity any) string {
	...
	case string:
		...
		return deriveStableUUIDv4("sub2api:codex-session-id:v2:" + value)
	default:
		return ""
	}
}
...
func resolveConvergedThreadID(identity any, clientSessionID string) string {
	if clientSessionID == "" {
		return ""
	}
	switch value := identity.(type) {
	case *Account:
		...
		return deriveStableUUIDv4(fmt.Sprintf("sub2api:codex-thread-id:v1:%d:%s", value.ID, clientSessionID))
	case string:
		if value == "" {
			return ""
		}
		return deriveStableUUIDv4("sub2api:codex-thread-id:v2:" + value + ":" + clientSessionID)
	default:
		return ""
	}
}
...
// resolveCodexFingerprintIDsForScope derives the converged identity for one tenant scope.
// Explicit device IDs remain account-wide; generated IDs are scoped before derivation so
// tenants sharing an OAuth account do not also share an upstream device/session identity.
// An empty scope preserves the account-level v2 behavior.
