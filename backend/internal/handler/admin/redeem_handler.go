...
type GenerateRedeemCodesRequest struct {
	...
	Type         string  `json:"type" binding:"required,oneof=balance concurrency subscription invitation token"`
...
type CreateAndRedeemCodeRequest struct {
	...
	Type         string  `json:"type" binding:"omitempty,oneof=balance concurrency subscription invitation token"` // 不传时默认 balance（向后兼容）
...
func (h *RedeemHandler) CreateAndRedeem(c *gin.Context) {
	...
	if req.Type == "subscription" || req.Type == "token" {
		if req.GroupID == nil {
			response.BadRequest(c, "group_id is required for subscription/token type")
			return
		}
	}
