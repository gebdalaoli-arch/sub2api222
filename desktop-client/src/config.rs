...
pub fn is_local_debug_api_base_url(api_base_url: &str) -> bool {
    let normalized = api_base_url.trim().trim_end_matches('/').to_ascii_lowercase();
...
pub fn packaged_local_debug_api_message(
    ...
    if is_debug_build
        || !uses_fallback_api_base_url
        || !is_local_debug_api_base_url(api_base_url)
    {
        return None;
    }
...
mod tests {
    ...
    fn localhost_detection_accepts_known_dev_origins() {
        ...
        assert!(!is_local_debug_api_base_url("https://api.example.com/api/v1"));
    }
    ...
    fn packaged_localhost_fallback_surfaces_actionable_message() {
        ...
        assert!(message.unwrap().contains("build-desktop-installer.ps1 -ApiBaseUrl"));
    }

    #[test]
    fn debug_build_or_explicit_backend_does_not_surface_packaging_message() {
        assert!(
            packaged_local_debug_api_message(DEFAULT_API_BASE_URL, true, true).is_none()
        );
        assert!(
            packaged_local_debug_api_message(DEFAULT_API_BASE_URL, false, false).is_none()
        );
        assert!(
