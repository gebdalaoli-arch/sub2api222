...
3. 计费入口：支持 CDK 兑换、订阅摘要与明细、订单明细、余额充值、套餐下单与支付入口追踪。
4. 消费明细：支持查看最近请求的模型、Token 消耗、请求时间和费用。
...
当前这版已经可以作为商业交付版桌面客户端使用；若服务端接口还在演进，桌面端会先按既定契约落代码并保留手动联调空间。

如果后续要继续套壳、交接研发或接手部署，请继续看：

- [桌面客户端交接文档](D:/挣钱/token/token客户端/客户端壳子/docs/superpowers/handoff/2026-04-19-desktop-client-handoff.md)
- [VMware Desktop GUI 补测记录](D:/挣钱/token/token客户端/客户端壳子/docs/superpowers/verification/2026-04-19-desktop-gui-vm-test.md)
- [服务器增量更新说明](D:/挣钱/token/token客户端/客户端壳子/docs/superpowers/deploy/2026-04-19-server-incremental-update-cn.md)
...
这部分当前已经提供“摘要 + 明细 + 下单 + 支付追踪”的统一工作台，消费明细则拆为独立页面。
...
- 平台代理 Desktop：当前改为 shell 启动 + 用户 `~/.codex` 受管配置注入，确保 Windows Store 版可读到完整运行契约
- 订单/订阅详情页：已接入桌面端明细视图
- 充值：已接入桌面端下单与支付入口打开逻辑，服务端未补齐的增强能力可后续对接
...
- 当前主打支持对象是 Codex CLI，同时保留 Codex Desktop 的商业化启动入口
- 只支持 active OpenAI 分组进入 Codex 相关平台模式
- Anthropic 分组不会再默认作为 Codex 启动目标

### 11.3 计费边界

- 已有摘要、明细、充值/订阅下单入口与支付状态刷新
- 若服务端临时未返回完整支付能力，桌面端会保留已完成的 UI 和契约层等待手动补后端
