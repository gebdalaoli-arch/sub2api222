...
impl StartupDiagnostics {
    pub fn initialize() -> Self {
        ...
        match detect_system_fingerprint() {
            Some(fingerprint) => {
                diagnostics.log(format!("system fingerprint: {}", fingerprint.summary()));
                if should_force_software_renderer(current_backend.as_deref(), &fingerprint) {
                    ...
                    diagnostics.log(format!("keeping user supplied SLINT_BACKEND={backend}"));
                } else {
                    diagnostics.log("keeping default Slint renderer selection");
                }
            }
            None => diagnostics
                .log("system fingerprint unavailable; keeping default Slint renderer selection"),
        }
    ...
    fn install_panic_hook(&self) {
        ...
        std::panic::set_hook(Box::new(move |panic_info| {
            ...
            let _ = append_log(&log_path, &format!("panic at {location}: {payload}"));
            previous(panic_info);
        }));
    }
}
...
pub fn should_force_software_renderer(
    ...
    VM_MARKERS.iter().any(|marker| normalized.contains(marker))
}
...
fn detect_system_fingerprint() -> Option<SystemFingerprint> {
    ...
    Some(SystemFingerprint {
        ...
        baseboard_manufacturer: bios.get_value("BaseBoardManufacturer").unwrap_or_default(),
