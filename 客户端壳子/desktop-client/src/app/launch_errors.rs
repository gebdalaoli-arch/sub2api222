pub fn describe_platform_launch_error(error_text: &str) -> String {
    ...
    if normalized.contains("windows_store_desktop_platform_unsupported") {
        return "当前安装包仍使用旧的 Windows 商店版限制逻辑。请升级到新的桌面客户端安装包后再试。"
            .to_string();
    }
    ...
    if normalized.contains("only allows codex official clients") {
        return "当前分组仅允许 Codex 官方客户端，请切换官方模式或更换分组。".to_string();
    }
...
mod tests {
    ...
    fn describe_platform_launch_error_handles_windows_store_desktop_limit() {
        let message = describe_platform_launch_error("WINDOWS_STORE_DESKTOP_PLATFORM_UNSUPPORTED");
