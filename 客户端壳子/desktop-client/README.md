...
如果你是后续接手人，建议同时阅读工程交接文档：

- [docs/superpowers/handoff/2026-04-19-desktop-client-handoff.md](D:/挣钱/token/token客户端/客户端壳子/docs/superpowers/handoff/2026-04-19-desktop-client-handoff.md)
- [docs/superpowers/verification/2026-04-19-desktop-gui-vm-test.md](D:/挣钱/token/token客户端/客户端壳子/docs/superpowers/verification/2026-04-19-desktop-gui-vm-test.md)

## 本地运行

推荐从 `客户端壳子` 目录运行隐藏窗口入口：
...
- Windows 边界：Windows Store 安装的 Codex Desktop 官方模式仍使用 `shell:AppsFolder`；平台代理模式当前已改为 shell 启动路径并注入独立 `CODEX_HOME`，用于绕开 VMware 客机中直接执行 `WindowsApps\...\app\Codex.exe` 触发的 `os error 5`。
